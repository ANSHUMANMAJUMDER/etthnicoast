    ...
    <div class="content">
        ...
        <div class="page-header">
            <div class="page-title">
                <h4>Etthnicoast Worlds</h4>
                <h6>Manage Etthnicoast Worlds</h6>
            </div>

            <div class="page-btn">
                <a class="btn btn-added" data-bs-toggle="modal" data-bs-target="#addEtthnicoastWorldModal">
                    <img src="{{ asset('public/assets/img/icons/plus.svg') }}" class="me-2">Add Etthnicoast World
                </a>
            </div>
        </div>
        ...
        <div class="card mb-3">
            <div class="card-body">
                <form method="GET" action="{{ route('etthnicoast-worlds.index') }}">
                    <div class="row g-2 align-items-center">
                        <div class="col-md-4">
                            <input type="text" name="q" value="{{ $q ?? '' }}" class="form-control" placeholder="Search title or subtitle...">
                        </div>
                        <div class="col-md-2">
                            <button class="btn btn-primary w-100" type="submit">Search</button>
                        </div>
                        <div class="col-md-2">
                            <a href="{{ route('etthnicoast-worlds.index') }}" class="btn btn-light w-100">Reset</a>
                        </div>
                    </div>
                </form>
            </div>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tbody>
                            @if($etthnicoastWorlds->count())
                                @foreach($etthnicoastWorlds as $row)
                                    <tr>
                                        <td>{{ ($etthnicoastWorlds->currentPage()-1) * $etthnicoastWorlds->perPage() + $loop->iteration }}</td>
                                        ...
                                        <td>
                                            <form action="{{ route('etthnicoast-worlds.toggleStatus', $row->id) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('PATCH')
                                                @if($row->is_active)
                                        ...
                                        <td class="text-end">
                                            ...
                                            <a href="javascript:void(0);"
                                               class="deleteBtn"
                                               data-delete-url="{{ route('etthnicoast-worlds.destroy', $row->id) }}"
                                               data-bs-toggle="modal"
                                               data-bs-target="#deleteEtthnicoastWorldModal">
                            ...
                            @else
                                <tr>
                                    <td colspan="6" class="text-center">No Etthnicoast Worlds Found</td>
                                </tr>
                            @endif
                        </tbody>
                    </table>
...
{{-- ADD MODAL --}}
<div class="modal fade" id="addEtthnicoastWorldModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">

            <div class="modal-header">
                <h5 class="modal-title">Add Etthnicoast World</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>

            <form action="{{ route('etthnicoast-worlds.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
...
{{-- EDIT MODAL --}}
<div class="modal fade" id="editEtthnicoastWorldModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">

            <div class="modal-header">
                <h5 class="modal-title">Edit Etthnicoast World</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
...
{{-- DELETE MODAL --}}
<div class="modal fade" id="deleteEtthnicoastWorldModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">

            <div class="modal-header">
                <h5 class="modal-title text-danger">Delete Etthnicoast World</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>

            <div class="modal-body">
                <p class="mb-0">Are you sure you want to delete this Etthnicoast World?</p>
...
document.addEventListener('DOMContentLoaded', function () {
    new bootstrap.Modal(document.getElementById('addEtthnicoastWorldModal')).show();
});
@endif
</script>
@endsection